...
export class ConsultationComponent {
  ...
  Bilan = [{ name: 'Bilan1', date: '24/10/2024', editing: false }];
  isNameValid = true;
  // Add a new bilan
  addBilan(): void {
  ...
  Ordonnance = [{ name: 'Ordonnance1', date: '24/10/2024', editing: false }];
  showPopup = false;
  // Add a new bilan
  addOrdonnance(): void {
    const today = new Date();
  ...
  onCategoryChange() {
    if (this.selectedTestCategory) {
      this.subcategories = Object.keys(
        TestCategories[this.selectedTestCategory] || {}
      );
    } else {
      this.subcategories = [];
    }
    ...
    if (this.selectedTestCategory && this.selectedSubCategory) {
      ...
      if (!this.selectedSubCategories.includes(this.selectedSubCategory)) {
        ...
        const subcategoryTests =
          TestCategories[this.selectedTestCategory]?.[this.selectedSubCategory];
        if (subcategoryTests) {
          this.selectedTests[this.selectedSubCategory] =
            subcategoryTests.reduce(
              (acc: { [key: string]: boolean }, test: string) => {
